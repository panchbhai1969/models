...
flags.DEFINE_string('input_dir','','Path to images you want to test')
...
TEST_IMAGE_PATHS = [ os.path.join(FLAGS.input_dir, file_name) for file_name in sorted(os.listdir(FLAGS.input_dir))]
print("Loading Done..........")
#############HELPER FUNCTIONS###################
...
def result_to_json(image_path,analysed_output_dir,scores,stages,detections,index_map,json_dir):
    ...
    json_name=os.path.join(json_dir,image_name.rsplit('.', 1)[0]+'.json')
...
def single_stage_class():
  ...
  for image_path in TEST_IMAGE_PATHS:
      ...
      result_to_json(image_path,FLAGS.out_images_dir,scores,stages,detections,category_index,FLAGS.out_json_dir)
...
def two_stage_class():
  ...
  for image_path in TEST_IMAGE_PATHS:
      ...
      if FLAGS.save_analysed_images:
        output_dir=FLAGS.out_images_dir
        cv2.imwrite(os.path.join(output_dir,image_name),analysed_image)
      result_to_json(image_path,FLAGS.out_images_dir,scores,stages,detections,category_index_stages,FLAGS.out_json_dir)
  print("Detections Done")
